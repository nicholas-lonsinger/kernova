...
/// Unit tests for `AppDelegate.reopenPresentation(hasOnScreenUserWindow:)` —
...
@Suite("AppDelegate.reopenPresentation", .admissionGated)
struct AppDelegateReopenPresentationTests {
    @Test("A reopen with a window already on screen presents nothing")
    func windowOnScreenPresentsNothing() {
        #expect(AppDelegate.reopenPresentation(hasOnScreenUserWindow: true) == .nothing)
    }

    @Test("A reopen with no window on screen presents the library")
    func noWindowOnScreenPresentsLibrary() {
        #expect(AppDelegate.reopenPresentation(hasOnScreenUserWindow: false) == .library)
    }
}
