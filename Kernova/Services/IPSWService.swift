...
final class IPSWService: Sendable {
    ...
    private func performDownload(
        ...
        // A completed IPSW with no resumable bundle means a prior attempt already
        // downloaded the file and failed later, in the install phase. The guard is
        // `isResumable`, not `exists`: a husk left by a finalize whose disposal
        // failed has no bytes to resume from and must not force a re-download of a
        // file already sitting complete at the destination.
    ...
    /// its stored `originalURL` differs from `remoteURL` (Apple shipped a new
    /// macOS build between attempts).
    ...
    /// Location of the in-progress `.kernovadownload` bundle for a final-IPSW destination.
    ...
    func streamBytes(
        ...
        do {
            ...
            // a pre-wrapped `IPSWError` would be caught below and double-wrapped.
...
/// `Contents/`: `Info.plist` (`IPSWDownloadMetadata`) and `data`, whose file size
/// IS the resume offset. The app's `Info.plist` declares `.kernovadownload` as a
/// package conforming to `com.apple.package`, so Finder renders the directory as
/// a single icon.
struct IPSWBundle: Sendable {
    ...
    /// with `IPSWService.discardResumeData(at:)`, so a disposal failure can't undo
    /// a download whose bytes are already in place.
...
/// a time, which for a multi-GB IPSW is billions of async iterations. The
...
enum IPSWError: LocalizedError {
    ...
    /// The existing IPSW (or its in-progress bundle) could not be trashed to make
    /// way for a "Download & Replace".
    case freshDownloadCleanupFailed(path: String, underlying: any Error)
    /// The install context's `downloadDestinationPath` does not name an `.ipsw` file.
