...
final class VsockClipboardService: ClipboardServicing {
    ...
    /// guest sends its `Hello`. Reset on disconnect.
    private(set) var agentVersion: String?

    /// Whether the guest agent is missing, current, or outdated relative to the
    /// bundled binary. Drives install/update affordances in the UI.
    var agentStatus: AgentStatus {
        ...
        // We only flag .outdated when installed is strictly older than bundled.
        // Newer-than-bundled (development builds, downgraded host) shows as
        // .current — telling a user with a newer agent that they're outdated
        // would be wrong.
    ...
    /// The agent version this build of the host bundles. Injected so tests can
    /// vary it; defaults to `KernovaGuestAgentInfo.bundledVersion` (sidecar
    /// generated by the "Package Guest Agent DMG" build phase).
    ...
    private func handle(frame: Frame) {
        ...
        case .hello(let hello):
            ...
            Self.logger.notice(
                "Guest clipboard agent connected for '\(self.label, privacy: .public)' (service=\(hello.serviceVersion, privacy: .public), agent=\(reportedVersion, privacy: .public), caps=\(hello.capabilities.joined(separator: ","), privacy: .public))"
            )
