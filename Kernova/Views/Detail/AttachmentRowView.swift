...
/// button and hands them in; this view owns only the editable title and the
/// rename state machine. The context menu is supplied lazily via ``contextMenu``
/// so it reflects current state at click time. `itemID` is the backing model's id
/// (a `StorageDisk` or `RemovableMediaItem`).
@MainActor
final class AttachmentRowView: NSView, NSTextFieldDelegate {
    ...
    /// width that would stretch the detail pane.
    ...
    func update(
        ...
        if !isRenaming {
            ...
            // Guarded with the title writes: `beginRename()` disables the
            // recognizer for the duration of an edit, so re-enabling it here
