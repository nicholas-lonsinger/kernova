...
struct CLIExitCodeTests {
    ...
    @Test("An unanswered guest account exits 5, with no flag hint")
    func guestAccountRefusalCarriesNoHint() {
        let message = "\u{201C}Alpha\u{201D} creates the macOS account."
        let refused = VMCommandResponse(
            result: .failure(
                .guestAccountPasswordRequired(
                    prompt: GuestAccountPrompt(
                        vm: vm, username: "ada", fullName: "Ada Lovelace", message: message))))
        do {
            ...
            // The wire's start carries no way to answer for the account, so
            // this tool adds no flag hint — the core's own message is all of it.
