...
/// code signature.
///
/// Used to pin an XPC peer requirement to "the same team as whoever signed me"
/// instead of a hardcoded team ID, which stays correct across clones and under
/// both Debug (Apple Development) and Release (Developer ID) signing.
public enum KernovaCodeSignature {
    private static let logger = Logger(subsystem: "app.kernova", category: "CodeSignature")

    /// Computed once: the running process's own signature can't change.
    ...
    private static func resolveTeamIdentifier() -> String? {
        ...
        // The key is absent (not present-but-nil) for unsigned or ad-hoc signed
        // code, which some test-host configurations expect — not a failure.
