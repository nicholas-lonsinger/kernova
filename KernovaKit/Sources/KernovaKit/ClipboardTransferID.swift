...
///
/// The id is a function of `(generation, repIndex, direction)` alone: a re-fire
/// for the same rep, or the window's preview fetch of it, lands on the same id
/// and so *joins* the `LazyPullCoordinator`'s in-flight pull rather than opening
/// a second one. A reused id's worst case is one spurious re-abort, never
/// corruption
