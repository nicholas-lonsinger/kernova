...
**Total: 67 source files + 2 helpers, 37 test files (29 suites + 8 mocks + 1 test-helpers).**
...
**Files:** 14 SwiftUI views + 6 AppKit views/controllers + 1 AppKit style/atom-factory file + 2 SwiftUI↔AppKit anchor bridges + 1 SwiftUI-shim AppKit button (`InfoButton`) across 4 subdirectories. Four popovers are now end-to-end AppKit: missing-attachment (`AttachmentIcon` shim → `MissingAttachmentPopoverContentViewController`), Storage Disk Create (`CreateStorageDiskPopoverAnchor` → `DiskSizePopoverContentViewController`), Removable Media Create (`CreateRemovableMediaPopoverAnchor` → `DiskSizePopoverContentViewController`), and the generic info-popover surface used at 10 call sites inside `VMSettingsView` and `StorageDiskReorderSheet` (`InfoButton` → `InfoPopoverContentViewController`). Each uses a real `NSPopover` via `PopoverPresenter`; controllers build their full layouts in `loadView()` using shared `CalloutStyle` tokens + `makeCalloutHeadline`/`makeCalloutBody`/`makeCalloutCode` atom factory functions. **No shared callout container or base class** — visual consistency comes from shared tokens, not inheritance. **Genuinely shareable controllers are reused via init parameterization** (`DiskSizePopoverContentViewController` serves both Storage Disk and Removable Media via `headline` + `caption` strings supplied at init; `InfoPopoverContentViewController` takes `[InfoPopoverParagraph]` where the typed enum distinguishes `.body` from `.code` snippets). AppKit controllers decouple from `VMLibraryViewModel` via delegate protocols; the SwiftUI/AppKit bridge representables implement the delegates and forward user choices to the view model. The only popover left in SwiftUI is the mic-permission popover (also in `VMSettingsView`), which will move to its own concrete `NSViewController` subclass in a follow-up PR. `CalloutBody` and `CalloutView.swift` are now used only by that mic-permission popover and will be deleted when it migrates.
