...
│   ├── VMBundleLayout.swift            # Sendable struct centralizing file paths within a .kernova bundle; `diskOnDiskBytes(forRelativePath:isInternal:)` returns any disk's on-disk footprint (`totalFileAllocatedSize`); `diskCapacityBytes(forRelativePath:isInternal:)` returns any disk's virtual capacity — an ASIF's from its `shdw` header (offset 0x30, big-endian 512-sectors; validated + bounds-checked), or the logical file size for a non-ASIF (raw `.img`/`.iso`/`.dmg`) — both read live, no caching
