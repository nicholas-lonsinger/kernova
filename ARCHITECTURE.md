...
- "Rename…" — always routes to the **sidebar** inline rename (`VMLibraryViewModel.renameVMInSidebar`), never the settings pane’s Name field: the sidebar row is the one rename surface visible in every VM state, while a running VM’s settings pane sits hidden behind the live display, so targeting it would make the command appear to do nothing. The settings Name field remains the click-to-rename affordance for the detail surface itself
