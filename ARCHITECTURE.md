...
│   │   ├── SidebarAgentStatusButton.swift # SwiftUI `NSViewRepresentable` shim — preserves the existing `(vmName:status:onMount:onDismiss:)` call surface used by `VMRowView` while delegating all rendering and popover work to AppKit
