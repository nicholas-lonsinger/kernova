...
- **`@MainActor` services** (interact with `VZVirtualMachine`):
  - `VirtualizationService` — start, stop, pause, resume, save state, restore state. `start(_:)` has three branches: restore from a save file, **boot in-place when `instance.virtualMachine` is already attached** (the post-install hand-off — see below), and the normal cold-boot path that builds a fresh `VZVirtualMachineConfiguration` and attaches a new VM. The hand-off branch exists because rebuilding a fresh `VZMacAuxiliaryStorage(contentsOf:)` on the same actor turn that just released the install-side instance races VZ's file lock on the auxiliary storage file ("Failed to lock auxiliary storage" error).
  - `MacOSInstallService` — loads restore image, creates platform files (aux storage, hardware model, machine identifier), runs `VZMacOSInstaller` with KVO progress tracking. On a **successful** install it does **not** tear down the VM: `instance.virtualMachine` stays attached and `status = .stopped` so the caller (typically `VMLibraryViewModel.installAndAutoBoot`) can chain a `start(_:)` that hits the in-place boot branch above. Matches Apple's canonical `VZMacOSInstaller` flow (start the same VM that was passed to the installer).
