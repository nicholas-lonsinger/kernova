...
│   ├── AppDelegate.swift               # NSApplicationDelegate — startup, window tracking, menu (incl. Settings ⌘, and the Option-alternate "Start in Recovery Mode"), suspend-on-quit
...
├── Utilities/
│   ├── AppPreferences.swift            # App-wide prefs over `UserDefaults` (injectable for tests); `alwaysShowAdvancedOptions` gates the Option-reveal of advanced menu actions
...
- "Start in Recovery Mode" — an advanced action surfaced as an Option-alternate of "Start" in both the Virtual Machine menu (⌥⌘R) and the sidebar context menu, gated on `VMInstance.canStartInRecovery` (stopped macOS guests). `menuNeedsUpdate(_:)` flips the menu item's `isAlternate` from `AppPreferences.alwaysShowAdvancedOptions` so it becomes an always-visible item when that preference is on. Selecting it routes through `VMLibraryViewModel.confirmStartInRecovery` → a `DetailAlertsPresenter` confirmation → `startInRecoveryConfirmed`
- "Force Stop" — surfaced two ways. In the **sidebar context menu** (`SidebarViewController.buildContextMenu`), when a VM is gracefully stoppable (`VMInstance.canStop`) it is the Option-alternate of "Stop", gated on `AppPreferences.alwaysShowAdvancedOptions` (same reveal mechanic as recovery, but no top anchor is needed — a Pause/Resume item always precedes the Stop pair). For transient states where graceful stop is unavailable (`canForceStop && !canStop`: starting/saving/restoring) it appears as a standalone, always-visible item. In the **Virtual Machine menu bar** it stays a plain always-visible sibling of "Stop" (validated by `canForceStop`) rather than an alternate, because — unlike recovery, where the primary and alternate share enablement — Stop is disabled during those transient states while Force Stop is the only working action, so the static menu must not bury it behind Option. Selecting it routes through `VMLibraryViewModel.confirmForceStop` → a `DetailAlertsPresenter` confirmation → `forceStopConfirmed`
