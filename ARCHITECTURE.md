...
│   ├── VMBundleLayout.swift            # Sendable struct centralizing file paths within a .kernova bundle; `diskOnDiskBytes(forRelativePath:isInternal:)` returns any disk's on-disk footprint (`totalFileAllocatedSize`); `asifCapacityBytes(forRelativePath:isInternal:)` reads an ASIF's virtual capacity from its `shdw` header (offset 0x30, big-endian 512-sectors; validated + bounds-checked) — both read live, no caching
...
│   │   ├── StorageDiskRowView.swift     # `NSView` for one Storage Disks row: the standard attachment layout plus inline title rename (double-click; mirrors `SidebarVMRowCellView`'s field-editor state machine, and commits on any click outside the field via a local mouse-down monitor) and a per-row right-click menu via `menu(for:)` → controller-supplied closure. Carries `diskID` + `infoAnchor` (the icon, so Get Info points at it) + `subtitleField` (so the controller re-populates the live size off-main on each refresh); the controller wires rename began/committed/cancelled closures
│   │   ├── StorageDiskSubtitle.swift   # `nonisolated diskSubtitle(for:bundleLayout:)` (reads `diskOnDiskBytes` + `asifCapacityBytes` **live**, so it can run on a detached task — no main-thread file I/O) + `@MainActor populateDiskSubtitle(_:for:bundleLayout:isMissing:)` which reads off-main and fills the label in (tagging the field's `identifier` with the disk id so a recycled Boot Order cell ignores a stale result and a same-disk refresh updates in place). The "In-bundle disk image" placeholder is **deferred** behind a ~100ms grace and cancelled when the read lands, so the sub-ms common case never flickers it — only a genuinely slow read surfaces it + `diskIconSystemName(for:)`. In-bundle disks show `<on disk> / <allocated>` uniformly for every disk including the main one, reflecting current state (e.g. an external resize); falls back to on-disk-only when capacity isn't readable. External disks resolve synchronously to their path. Used by both the settings list and the Boot Order sheet
│   │   ├── StorageDiskInfoPopoverContentViewController.swift # Concrete `NSViewController` for the storage row's "Get Info" popover — headline (label) + two-column `NSGridView` of facts (file, on-disk, allocated, read-only, bus, created) + monospaced selectable full path, built in `loadView()` from `CalloutStyle`; value-snapshot init (no live `VMInstance`)
...
├── StorageDiskSubtitleTests.swift      # `diskSubtitle(for:bundleLayout:)` reading live from real on-disk files: ASIF → on-disk/allocated, non-ASIF → on-disk-only, missing → generic label, external → path, main disk measured identically
...
| `VMBundleLayout` (disk sizing) | `VMBundleLayoutTests` | `diskOnDiskBytes` nil/size/sparse; `asifCapacityBytes` parses a crafted `shdw` header (sector count → bytes) and returns nil for non-ASIF |
| `diskSubtitle(for:bundleLayout:)` | `StorageDiskSubtitleTests` | Reads live from real files: ASIF disk → on-disk/allocated; non-ASIF → on-disk-only; missing file → "In-bundle disk image"; external → path; main disk measured identically to additional disks |
