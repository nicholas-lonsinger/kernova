...
│   │   ├── StorageDiskSubtitle.swift   # Shared `diskSubtitle(for:in:)` / `diskIconSystemName(for:)` free functions used by both the settings list and the AppKit Boot Order sheet
...
├── DeleteVMSheetContentViewControllerTests.swift # Header shows VM name; in-bundle disks render read-only under their section; one row per external; exclusively-owned externals start checked; shared externals are locked off with a "Kept" note; Move-to-Trash returns the checked externals' ids; Cancel fires delegate; Move-to-Trash is the default+destructive button; Cancel is keyed to Escape
