...
| `VMInstance` | Yes | Status transitions, configuration updates, bundle layout, preparing state display properties, post-start agent watchdog (firing/cancellation/no-op guards/idempotency), `recordObservedAgentVersion` persistence + dedup, `canStartInRecovery` (stopped macOS only; `VMInstanceRecoveryEligibilityTests`) |
