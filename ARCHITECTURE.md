...
- The application menu (including VM-specific actions, Force Stop with `canForceStop` validation, and Window > Show Library with Cmd+0). The View menu hosts the toolbar commands — "Show/Hide Toolbar" (⌥⌘T, `NSWindow.toggleToolbarShown(_:)`) and "Customize Toolbar…" (`NSWindow.runToolbarCustomizationPalette(_:)`) — as nil-target standard actions that resolve against the key window, so AppKit retitles/validates them without any `validateMenuItem` involvement; AppKit also auto-inserts "Enter Full Screen" there (#318, #319)
