...
**A bridge call on the main thread runs a nested event loop, and one entered from a main-queue job cannot drain the main queue.** CFRunLoop does not service the main dispatch queue from a loop nested inside a main-queue callout, and a `@MainActor` test body *is* a main-queue job.
So a test that needs main-queue work — a progress hop, an observation, another `MainActor` continuation — to run *inside* the pull enters the bridge from a main-run-loop callout (`RunLoop.main.perform`), the context production's pasteboard callback arrives in. A call made directly from the test body still resolves (the wake is run-loop-level) but holds the bundle's `MainActor` for the pull's duration.
