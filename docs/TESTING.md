...
Canonical bug: `FileProviderServiceSourceTests.cancelDispatchedPullAsksOwnerToAbort` once passed `fetchReplyTimeout: 2` to bound a harmless post-`cancel()` timer; on a stalled runner that reply timer fired *before* `cancel()`, replacing the expected `NSUserCancelledError` with `serverUnreachable`.
