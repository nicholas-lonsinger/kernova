...
- A panel's pinned header is one row: an image-only back chevron leading the category title, the VM's status facts trailing. Its well is drawn — an always-visible `CornerRadius.control` fill — because every stock bezel at that size appears only under the pointer. The chevron matches the overview cards' at `.secondaryLabelColor`, never accent-tinted, so both arrows read as one family.
