...
- The pinned header has one anatomy in both states — icon tile, title, status dot over a facts line — so drilling into a category morphs it in place instead of swapping in a second view. The tile is unchanged and only its glyph turns from the guest OS's into a `chevron.left` at the same size, `.secondaryLabelColor` and never accent-tinted; the title turns from the VM's name into the category's, and a category's affordances sit beside it.
