...
- The detail pane is an overview of one card per `VMSettingsCategory`, stacked down the column in category order, each drilling into that category's panel behind an accent "Edit" affordance. A card states the facts answering "what is this VM right now" — never the panel's full row list, and never a fact the header's own line already carries — so a new setting decides its card representation, or states that it has none.
- The pinned header is one anatomy in both states: an icon tile, a title beside it, and a status dot over a facts line below that title. Drilling into a category morphs it in place — the tile's glyph turns from the guest OS's into a `chevron.left` in `.controlAccentColor`, and the title from the VM's name into the category's.
...
- A card states its lock as a `lock.fill` beside text naming only the rows that actually lock ("Disks editable when stopped"). Scoping the claim is what lets it stand beside live controls on the same card; a panel's own section headers claim the whole section instead.
