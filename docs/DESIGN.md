...
- The detail pane is an overview of one card per `VMSettingsCategory`, paired two to a row in category order, each drilling into that category's panel behind an accent "Edit" affordance. A card states the facts answering "what is this VM right now" — never the panel's full row list, and never a fact the header's own line already carries — so a new setting decides its card representation, or states that it has none.
- The pinned header states the VM on the overview — icon tile, name, status dot over a facts line — and the open category on a single row: a small bezeled `chevron.left` button in `.controlAccentColor`, the category's title, its affordances beside it, and the same status dot and facts line trailing that row. One view morphs between the two rather than a second one swapping in.
- A boolean on a card renders as its own live switch, dimmed through `applyGroupedFormRowEnabled` while the current state doesn't allow editing it.
- A card states its lock as a `lock.fill` glyph alone, whose tooltip names only the rows that actually lock ("Disks editable when stopped"). Scoping the claim is what lets it stand beside live controls on the same card; a panel's own section headers carry the full-text hint, where a whole section locks.
