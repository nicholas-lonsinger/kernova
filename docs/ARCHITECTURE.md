...
- `VMCommandCore` — the headless implementation of every VM verb, beneath the UI and every
  ...
  enablement and every one of the core's verb guards derive from it. Clone and import register a
  preparing "phantom" `VMInstance` **synchronously, before any `await`** — that is what reserves
  the destination atomically on the MainActor, so overlapping imports and clones cannot claim the
  same bundle URL.
