...
- **Feature-channel admission is gated on the per-VM control handshake (#145).** The log and
  clipboard listeners refuse a connection while no control channel with a completed `Hello`
  exists for the VM (the clipboard listener additionally requires the negotiated
  `clipboard.stream.v1`), and a feature channel that carries another port's payload is closed.
  A delivered per-VM secret is deliberately **not** part of the design: any secret the
  unprivileged guest agent could present is readable by every other unprivileged in-guest
  process — the very peer it would need to exclude — so it adds delivery and reinstall churn
  without adding defense. Revisit only if the guest ever gains an internal privilege boundary
  (untrusted workloads isolated away from the agent's files).
...
- **A reinstalled guest agent does not replace its running File Provider extension.**
  ...
  (a `pkill` matched on the appex executable path, confirmed with SIGKILL escalation;
  `fileproviderd` respawns the new binary on demand), so a scripted reinstall is safe — but when replacing the bundle any other way (Xcode build,
...
| **#145** — per-VM auth on the vsock listener | §10 Trust boundary | Required before non-trusted guest workloads; the host must authenticate the guest per VM. ✓ **Resolved** — feature-channel admission is gated on the per-VM control handshake: `VsockListenerHost`'s `shouldAdmit` predicate refuses a log/clipboard connection at accept while no control channel with a completed `Hello` exists (clipboard additionally requires the negotiated `clipboard.stream.v1`), a feature channel carrying another port's payload is closed, and peer-supplied `Hello.capabilities` strings are redaction-bounded before logging. The issue's delivered per-VM secret was deliberately rejected: any secret the unprivileged agent could present is readable by every other unprivileged in-guest process — the adversary it would need to exclude — so it adds delivery/reinstall churn without defense (§10); revisit only if an intra-guest privilege boundary appears. |
...
| **#634** — Finder's copy dialog is still indeterminate on an FP paste, despite a correctly advancing `fetchContents` `Progress` | §13 Legibility | The determinate signal exists; it just isn't reaching the surface the user watches. Fix at the surface, not by re-plumbing the transport (§5 — progress never touches the data path), and fix it in the shared owner-side relay so both directions are covered at once. **Fixed (live re-verification pending)** — the owner publishes a cross-process `NSProgress` keyed to the placeholder's user-visible file URL (`kind = .file`, `.downloading`, byte `totalUnitCount`) for the pull's duration, fed by the same per-chunk callback and coalesced by the same throttle, unpublished at every terminal, with the ~300 ms reveal gate the in-app bar already uses. The *mechanism* was verified live before implementation — a throwaway experiment publishing exactly this produced a determinate 4 % → 28 % → full bar over a ~38 s guest→host paste — but the shipped code has not itself been observed against a running VM, and the folder-tree case (each child publishes its own URL) is unverified in either form. Not marked resolved until both directions are re-observed; this row is the reason to keep that distinction visible, since the claim it replaces was written the same way. |
