...
Kernova is an ordinary resident menu-bar app — not a launchd agent, and there is no Mach service anywhere in the design. "Open at Login" is an opt-in General-settings toggle over `SMAppService.mainApp` (`LoginItemService`), which is MAS- and sandbox-compatible.
