...
Read [docs/REVIEW.md](docs/REVIEW.md) before filing review-debt issues or annotating: it has the full severity bar with worked examples, the issue template and `review-debt/*` labels, the issue-hygiene rules (cite symbols, never line numbers; keep bodies to what/why with no forward-looking implementation sketch — these apply to *every* issue you file), and the annotation formats.
