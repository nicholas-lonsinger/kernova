...
- **A uniform gap beats a path-dependent capability.** An improvement that can only be wired on one path, one direction, or behind an opt-in is worse than not shipping it: a gap uniform by construction closes, when it closes, for every path at once. Worked case: `ClipboardArchive.fieldKeys`.
