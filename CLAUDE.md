...
`ExitWorktree(remove)` deletes the `worktree-<name>` branch `EnterWorktree` created, by that name, so leave the scratch branch named as-is and put the PR head name AGENTS.md prescribes on the remote alone: `git push -u origin HEAD:<type>/<short-description>`.
