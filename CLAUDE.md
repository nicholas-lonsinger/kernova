...
The `-derivedDataPath DerivedData/Kernova` flag ensures build output goes to a deterministic local directory (the whole `DerivedData/` tree is already gitignored) instead of the per-path-hashed `~/Library/Developer/Xcode/DerivedData/` location. This avoids glob ambiguity when worktrees or parallel builds create multiple DerivedData folders. Xcode's own builds default to the per-user `~/Library/Developer/Xcode/DerivedData/` location and don't need to share this one; to make Xcode build into the same worktree-local directory, set Settings → Locations → Derived Data to *Relative*. Xcode's *Relative* mode nests a per-project subfolder rather than writing straight into the folder you point it at (`DerivedData/Kernova/`, not `DerivedData/`) — the CLI invocation above targets that same nested path so a terminal build and an Xcode build land in one shared build dir instead of two divergent copies. `make clean` removes the whole `DerivedData/` root, clearing both. This only unifies where the products land, not incremental build state: Xcode's own build request records its arena identity as the un-nested `DerivedData` path (an IDE-only nesting detail invisible to `-derivedDataPath`), so switching between an Xcode build and a CLI build still triggers a full rebuild rather than reusing the other's cache.

### Build version

`CFBundleVersion` is `git rev-list --count HEAD` (the total commit count), substituted into the source `Info.plist` via `INFOPLIST_PREPROCESS`. The `Set Build Number from Git` build phase generates `KernovaBuildNumber.h` with `#define KERNOVA_BUILD_NUMBER N`, and `Kernova/App/Info.plist` references the symbol directly (`<string>KERNOVA_BUILD_NUMBER</string>`). Substitution happens inside `ProcessInfoPlistFile` so build-graph reordering can't clobber it. The `KernovaMacOSAgent` target uses the same pattern with its own `AGENT_BUILD_NUMBER` (scoped to `git rev-list --count HEAD -- KernovaGuestAgent/ KernovaMacOSAgent/` — both the pre- and post-rename directory paths, so the count stays monotonic across the rename). The `KernovaQuickLook` extension replicates the app's pattern verbatim (unscoped total count → `KERNOVA_BUILD_NUMBER` in its own `DERIVED_FILE_DIR`) so the embedded appex's version always matches its host app. The `KernovaFileProvider` extension (the host "Copy to Mac" File Provider, also embedded in the app) replicates the same unscoped `KERNOVA_BUILD_NUMBER` pattern. The `KernovaMacOSAgentFileProvider` extension (embedded in the guest agent) shares the agent's `AGENT_BUILD_NUMBER` the same way — its own `Set Build Number from Git` phase + `AgentBuildNumber.h` — so its version always matches the agent. When adding a new top-level target that needs a dynamic build number, replicate this pattern instead of patching the built `Info.plist` after the fact.
