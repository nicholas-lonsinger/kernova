...
# executable pre-push, commit-msg and post-checkout hooks. Verifies reality
# rather than string-comparing the config value against ".githooks" — an
# absolute path (or any other spelling) that resolves to a working hooks
# directory counts as installed; the literal comparison used to produce false
# "not installed" nudges for exactly that setup.
#
...
for hook in pre-push commit-msg post-checkout; do
    [ -x "$hp/$hook" ] || exit 1
done
printf '%s\n' "$hp"
