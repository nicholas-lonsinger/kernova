...
Kernova is a pure-AppKit app that manages virtual machines via Apple's `Virtualization.framework`, supporting macOS and Linux guests. No SwiftUI anywhere — sidebar, detail pane, creation wizard, alerts, popovers, and sheets are all `NSViewController` / `NSWindowController` subclasses driven by `observeRecurring(track:apply:)` against `@Observable` view-model state.

**Data flow:** `AppDelegate` → `VMLibraryViewModel` → `VMLifecycleCoordinator` → services + AppKit views. `MainWindowController` hosts an `NSSplitViewController` with a source-list sidebar (`SidebarViewController` driving an `NSOutlineView`) and detail (`DetailContainerViewController` → `DetailRouterViewController`) panes, plus a native `NSToolbar` with `NSToolbarItem`s. The detail pane layers a pure-AppKit `VMDisplayBackingView` (for `VZVirtualMachineView`) over the router. `VMDirectoryWatcher` monitors the VMs directory for external filesystem changes and triggers reconciliation in the view model.
